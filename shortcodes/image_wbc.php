...
$wbcvpb_shortcodes['image_wbc'] = array(
...
function wbcvpb_image_wbc_shortcode( $attributes, $content = null ) {
	extract(shortcode_atts(wbcvpb_extract_attributes('image_wbc'), $attributes));
	$id_out = ($id!='') ? 'id='.$id.'' : '';
	...
	$return = '<div '.esc_attr($id_out).' class="wbcvpb-image '.esc_attr($classes).'" '.$animation_out.'>';

	if($lightbox == '1') {
		$return .= '<a href="'.esc_url($url).'" class="lightbox" data-lightbox="image"><img class="img-responsive" src="'.esc_url($url).'">
			<canvas class="grey-effect"></canvas>
			'.$lightbox_icon.'
		</a>';
	}
	else if($link != '') {
		$return .= '<a href="'.esc_url($link).'" target="'.esc_attr($target).'"><img class="img-responsive" src="'.esc_url($url).'"></a>';
			// $return .= ($overlay_text !='') ? '<canvas class="grey-effect"></canvas><span>'.$overlay_text.'</span>' :'';
		// $return .= '</a>';
	}
	else{
		$return .= '<img class="img-responsive" src="'.esc_url($url).'">';
	}
