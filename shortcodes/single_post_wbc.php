...
$wbcvpb_shortcodes['single_post_wbc'] = array(
	'name' => esc_html__('Single post', 'the-creator-vpb' ),
	'notice' => esc_html__('Display single post', 'the-creator-vpb'),
	'type' => 'block',
	'icon' => 'pi-sidebar',
...
function wbcvpb_single_post_wbc_shortcode( $attributes ) {
    extract(shortcode_atts(wbcvpb_extract_attributes('single_post_wbc'), $attributes));
	$id_out = ($id!='') ? 'id='.$id.'' : '';
	...
	$text = do_shortcode($postData->post_content);
	...
	$thumb = get_the_post_thumbnail($name,'full', array('class' => 'img-responsive'));

    return '<div '.$animation_out.' class="'.$classes.'" '.esc_attr($id_out).' '.esc_attr($class_out).' >
    	<div class="post-image">'.$thumb.'</div><p>'.do_shortcode($content).'</p><a class="more" href="'.$url.'">'.$url_text.'</a></div>';
}
