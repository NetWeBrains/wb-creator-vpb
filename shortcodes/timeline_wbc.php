...
$wbcvpb_shortcodes['timeline_wbc'] = array(
...
function wbcvpb_timeline_wbc_shortcode( $attributes, $content = null ) {
	global $tabs_navigation,$tabs_content,$tabs_counter,$tabs_selected;
	extract(shortcode_atts(wbcvpb_extract_attributes('timeline_wbc'), $attributes));
	static $i = 0;
    $i++;
	...
	$return = '
		<div '.esc_attr($id_out).' class="wbcvpb-tabs wbcvpb-tabs-'.esc_attr($slide_direction).' wbcvpb-tabs-position-'.esc_attr($tabs_position).' wbcvpb-tabs-timeline '.esc_attr($class).'" data-selected="'.esc_attr($selected).'" role="tabpane'.$i.'" data-break_point="'.esc_attr($break_point).'" data-effect="'.esc_attr($effect).'">
...
$wbcvpb_shortcodes['tl_time_wbc'] = array(
...
function wbcvpb_tl_time_wbc_shortcode( $attributes, $content = null ) {
	extract(shortcode_atts(wbcvpb_extract_attributes('tl_time_wbc'), $attributes));
	...
	$tabs_navigation.='<li role="presentation"'.(($active_class)?' class="active"':'').'><a class="wbcvpb-tabs-tab" data-href="#timeline-'.$tabs_counter.'-'.$tab_count.'" aria-controls="tab-'.$tabs_counter.'-'.$tab_count.'" role="tab" data-toggle="tab">'.$icon_output . $title . '</a></li>';
	$tabs_content.='
