...
$wbcvpb_shortcodes['gridposts_wbc'] = array(
	'name' => esc_html__('Grid Posts', 'the-creator-vpb' ),
	'type' => 'block',
	'icon' => 'pi-posts',
...
if ( ! function_exists( 'wbcvpb_gridposts_wbc_shortcode' ) ){
	function wbcvpb_gridposts_wbc_shortcode( $attributes ) {
		extract(shortcode_atts(wbcvpb_extract_attributes('gridposts_wbc'), $attributes));
		$id_out = ($id!='') ? 'id='.$id.'' : '';
		wp_reset_postdata();
		global $paged;
		...
		while ( $listing->have_posts() ): $listing->the_post();
			...
			$output .= '<div '.esc_attr($id_out).' class="'.$postclass.' wbcvpb_gridposts_shortcode wbcvpb_gridposts_shortcode-'.esc_attr($style).' clearfix'.esc_attr($hasthumb_class).' '.esc_attr($classes).'" '.$animation_out.'>';
			...
			$output .= '<div class="wbcvpb_gridposts_shortcode_content">';
